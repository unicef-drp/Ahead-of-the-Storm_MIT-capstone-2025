...
from src.impact_analysis.helper.surge_processor import SurgeProcessor
...
class SurgeLayer(ExposureLayer):
    ...
    def __init__(
        self,
        hurricane_file: str = None,
        bathymetry_file: str = None,
        config: dict = None,
        cache_dir: str = None,
        use_cache: bool = True,
    ):
        ...
        if hurricane_file is not None:
            ...
            if self.load_data():
                ...
                if self._load_cached_results():
                    ...
                    if self.results:
                        print(
                            f"Surge ensemble simulation completed with {len(self.results)} members"
                        )
    ...
    def load_data(
        self, hurricane_file: str = None, bathymetry_file: Optional[str] = None
    ) -> bool:
        ...
        self.grid_lats, self.grid_lons, self.grid_depths, self.dx, self.dy = (
            self.processor.get_grid_data()
        )
    ...
    def _load_hurricane_data(self, hurricane_file: str) -> bool:
        ...
        try:
            ...
            self.hurricane_data["valid_time"] = pd.to_datetime(
                self.hurricane_data["valid_time"]
            )

            print(f"Loaded {len(self.hurricane_data)} hurricane records")
            print(
                f"  Ensemble members: {self.hurricane_data['ensemble_member'].nunique()}"
            )
            print(
                f"  Time range: {self.hurricane_data['valid_time'].min()} to {self.hurricane_data['valid_time'].max()}"
            )
            ...
            missing_cols = [
                col for col in required_cols if col not in self.hurricane_data.columns
            ]
            if missing_cols:
            ...
            wind_radii_cols = [
                col
                for col in self.hurricane_data.columns
                if "radius_" in col and "knot_winds_" in col
            ]
            rmw_col = "radius_of_maximum_winds_km"

            if wind_radii_cols:
                print(
                    f"  Found {len(wind_radii_cols)} wind radii columns - will use observed asymmetric structure"
                )
            else:
            ...
            print(
                f"  Storm intensity: {max_wind:.0f}kt peak, {min_pressure:.0f}mb minimum"
            )
    ...
    def calculate_radius_max_winds(
        self, storm_data, max_wind_ms, central_pressure, storm_lat
    ):
        ...
        if "radius_of_maximum_winds_km" in storm_data and not pd.isna(
            storm_data["radius_of_maximum_winds_km"]
        ):
    ...
    def get_quadrant_from_bearing(self, bearing_rad):
        ...
        quadrant = np.where(
            ...
            0,  # NE
            np.where(
                bearing_deg < 180, 1, np.where(bearing_deg < 270, 2, 3)  # SE  # SW
            ),
        )  # NW
        return quadrant
    ...
    def interpolate_wind_from_radii(
        self, distance_km, wind_radii, quadrant, max_wind_speed
    ):
        ...
        if np.isscalar(quadrant):
            ...
            for i, quad_idx in enumerate(np.ravel(quadrant)):
                ...
                quad_radii = {ws: wind_radii[ws][quad_name] for ws in [34, 50, 64]}
                result.ravel()[i] = self._interpolate_single_point(
                    np.ravel(distance_km)[i], quad_radii, max_wind_speed
                )
            return result
    ...
    def _interpolate_single_point(self, distance_km, quad_radii, max_wind_speed):
        ...
        valid_radii = [
            (r, wind_ms)
            for r, wind_ms in [(r64, 32.9), (r50, 25.7), (r34, 17.5)]
            if r > 0
        ]
            ...
            for i, (radius, wind_thresh) in enumerate(valid_radii):
                if distance_km <= radius:
                    if i == 0:
                        ...
                        wind_speed_ms = max_wind_speed - (
                            max_wind_speed - wind_thresh
                        ) * (distance_km - rmw_approx) / (radius - rmw_approx)
                    else:
                        # Between two radii
                        prev_radius, prev_wind = valid_radii[i - 1]
                        wind_speed_ms = prev_wind - (prev_wind - wind_thresh) * (
                            distance_km - prev_radius
                        ) / (radius - prev_radius)
                    break
            else:
                # Outside all radii - exponential decay
                outermost_radius, outermost_wind = valid_radii[-1]
                wind_speed_ms = outermost_wind * np.exp(
                    -(distance_km - outermost_radius) / 50
                )
    ...
    def generate_realistic_hurricane_wind_field(self, storm_data, member, step):
        ...
        storm_speed, storm_direction = self.calculate_storm_motion(
        ...
        if wind_radii is not None:
            ...
            wind_speed_asymmetric = self.interpolate_wind_from_radii(
                distance_km, wind_radii, quadrant, max_wind_speed
            )

            # If interpolation failed, fall back to Holland model
            if wind_speed_asymmetric is None:
                wind_speed_asymmetric = self._generate_holland_wind_field(
                    storm_data,
                    distance_km,
                    bearing_rad,
                    max_wind_speed,
        ...
        rmw = self.calculate_radius_max_winds(
            storm_data, max_wind_speed, central_pressure, storm_lat
        )
    ...
    def _generate_holland_wind_field(
        self,
        storm_data,
        distance_km,
        bearing_rad,
        ...
        holland_b = self.calculate_holland_b_parameter(
        ...
        if pressure_deficit > 5:
            try:
                ...
                holland_term = (
                    holland_b * pressure_deficit * 100
                ) / 1.225  # Convert mb to Pa
                ...
                if max_wind_speed > 50:  # Hurricane strength
                    ...
                    wind_speed_symmetric = np.maximum(
                        wind_speed_symmetric, intensity_floor
                    )

                # Cap at reasonable maximum
                wind_speed_symmetric = np.minimum(
                    wind_speed_symmetric, max_wind_speed * 1.5
                )

            except:
                # Fallback
                wind_speed_symmetric = max_wind_speed * np.exp(
                    -distance_km / (rmw * 0.6)
                )
        else:
            # Weak system
        ...
        motion_asymmetry = 1.0 + 0.3 * (storm_speed / 10.0) * np.cos(
            relative_bearing - np.pi / 2
        )
        ...
        distance_asymmetry = 1.0 + 0.15 * np.exp(-distance_km / 80) * np.cos(
            relative_bearing
        )
        distance_asymmetry = np.clip(distance_asymmetry, 0.7, 1.5)

        # Apply all asymmetries
        wind_speed_asymmetric = (
            wind_speed_symmetric
            * motion_asymmetry
            * coriolis_effect
            * distance_asymmetry
        )
    ...
    def surge_physics(self, u_wind, v_wind, pressure_field, dt):
        ...
        pressure_acc_x, pressure_acc_y = self.calculate_pressure_gradient(
            pressure_field
        )
        ...
        # Dynamic pressure
        dynamic_setup = np.where(wind_speed > 15, wind_speed**2 * 0.00004, 0)
        ...
        surge_increment = (
            deta_dt * dt + static_setup * dt / 2400 + dynamic_setup * dt / 2400
        )
        ...
        if max_wind > 40:
            ...
            if np.any(coastal_mask):
                ...
                smoothed_coastal = gaussian_filter(
                    coastal_surge, sigma=1
                )  # Reduced smoothing
        ...
        inland_penetration_enabled = get_config_value(
            self.config, "surge.inland_penetration_enabled", False
        )

        if inland_penetration_enabled:
            # Get inland penetration parameters
            max_inland_distance = get_config_value(
                self.config, "surge.max_inland_distance_km", 5.0
            )
            inland_decay = get_config_value(
                self.config, "surge.inland_decay_factor", 0.8
            )
            coastal_elevation_threshold = get_config_value(
                self.config, "surge.coastal_elevation_threshold", 5.0
            )
            ...
            inland_mask = (self.grid_depths >= 0) & (
                distance_from_coast <= max_inland_distance
            )
                            ...
                            if distance_km <= max_inland_distance:
                                ...
                                if nearby_coastal_surge > 0:
                                    ...
                                    elevation_factor = max(
                                        ...
                                        1.0
                                        - (
                                            self.grid_depths[i, j]
                                            / coastal_elevation_threshold
                                        ),
                                    )
    ...
    def run_ensemble_simulation(
        self, n_members: Optional[int] = None, max_hours: Optional[int] = None
    ) -> Dict:
        ...
        if n_members is None:
            ...
            print(
                f"Using {len(members_to_run)} of {len(available_members)} available members"
            )
        ...
        for i, member in enumerate(members_to_run):
            ...
            member_data = self.hurricane_data[
                self.hurricane_data["ensemble_member"] == member
            ]
            ...
            for step in range(max_steps):
                try:
                    ...
                    # Generate wind field
                    u_wind, v_wind, pressure_field = (
                        self.generate_realistic_hurricane_wind_field(
                            storm_data, member, step
                        )
                    )
                    ...
                    if not self.surge_physics(
                        u_wind, v_wind, pressure_field, dt_seconds
                    ):
                    ...
                    if step % (21600 // dt_seconds) == 0 and step > 0:
                        ...
                        print(
                            f"    {step * dt_seconds / 3600:.0f}h: Surge={current_surge:.3f}m, "
            ...
            if success:
                ...
                coverage_percent = (
                    surge_10cm_area / total_water * 100 if total_water > 0 else 0
                )
                ...
                print(
                    f"    FINAL: Max={final_max_surge:.3f}m, Coverage={coverage_percent:.1f}%, "
    ...
    def get_surge_probability(self, threshold: float = 0.5) -> np.ndarray:
        ...
        if not hasattr(self, "results") or not self.results:
            return (
                np.zeros_like(self.grid_lats)
                if self.grid_lats is not None
                else np.array([])
            )
        ...
        try:
            ...
            if interpolated_surge is None:
                ...
                return gpd.GeoDataFrame(
                    {"probability": [0.0]}, geometry=[Point(0, 0)], crs="EPSG:4326"
                )

            # Use the EXACT same grid system as vulnerability layers
            from src.impact_analysis.layers.population import (
                PopulationVulnerabilityLayer,
            )
            from src.utils.hurricane_geom import get_nicaragua_boundary
            ...
            print(
                f"Exposure probability range: {np.min(grid_gdf['probability']):.3f} to {np.max(grid_gdf['probability']):.3f}"
            )
    ...
    def save_high_resolution_data(self, output_dir="data/preprocessed/surge/"):
        ...
        if self.grid_lons is not None and self.grid_lats is not None:
            try:
                ...
                transform = from_bounds(
                    ...
                    max_lon,
                    max_lat,
                    self.grid_lons.shape[1],
                    self.grid_lats.shape[0],
                )
                ...
                for member_id, result in self.results.items():
                    ...
                    member_tiff_file = (
                        output_path / f"nicaragua_surge_member_{member_id}.tif"
                    )
        ...
        metadata = {
            ...
            "grid_lons": (
                self.grid_lons.tolist() if self.grid_lons is not None else None
            ),
            "grid_lats": (
                self.grid_lats.tolist() if self.grid_lats is not None else None
            ),
            "grid_depths": (
                self.grid_depths.tolist() if self.grid_depths is not None else None
            ),
        }
    ...
    def _save_surge_exposure_visualization(
        self, output_path: Path, surge_exposure: np.ndarray
    ):
    ...
    def _save_interpolated_surge_data(self, output_path: Path):
        ...
        try:
            ...
            if interpolated_surge is not None:
                ...
                interpolated_file = (
                    output_path / "nicaragua_surge_interpolated_heights.npy"
                )
                np.save(interpolated_file, interpolated_surge)

                # Create visualization of interpolated surge heights
                self._create_interpolated_surge_visualization(
                    output_path, interpolated_surge
                )
                ...
                with open(stats_file, "w") as f:
                    f.write(
                        "Interpolated Surge Height Statistics (After Inland Penetration)\n"
                    )
                    f.write("=" * 60 + "\n\n")
                    f.write(f"Grid shape: {interpolated_surge.shape}\n")
                    f.write(
                        f"Maximum surge height: {np.max(interpolated_surge):.3f} m\n"
                    )
                    ...
                    f.write(
                        f"Cells with surge > 0.1m: {np.sum(interpolated_surge > 0.1)}\n"
                    )
                    f.write(
                        f"Cells with surge > 0.5m: {np.sum(interpolated_surge > 0.5)}\n"
                    )
                    f.write(
                        f"Cells with surge > 1.0m: {np.sum(interpolated_surge > 1.0)}\n"
                    )
                    f.write(
                        f"Cells with surge > 2.0m: {np.sum(interpolated_surge > 2.0)}\n"
                    )
                ...
                results_dir = Path(
                    "data/results/impact_analysis/surge_population_ensemble"
                )
    ...
    def _get_interpolated_surge_data(self):
        ...
        print(
            f"Significant surge points (> {surge_threshold}m): {len(comp_lons_surge)}"
        )

        # Simple surge extension approach
        max_extension_distance_km = get_config_value(
            self.config, "surge.max_inland_distance_km", 5.0
        )
        ...
        for i, (lon, lat) in enumerate(zip(grid_lons, grid_lats)):
            ...
            if len(comp_lons_surge) > 0:
                distances = (
                    np.sqrt((lon - comp_lons_surge) ** 2 + (lat - comp_lats_surge) ** 2)
                    * 111.0
                )  # Convert to km
                ...
                if nearest_distance_km <= max_extension_distance_km:
                    # Apply distance-based decay
                    decay_factor = 1.0 - (
                        nearest_distance_km / max_extension_distance_km
                    )
        ...
        print(
            f"Extended surge range: {np.min(extended_surge):.3f} to {np.max(extended_surge):.3f}"
        )
    ...
    def _get_raw_surge_data(self):
        ...
        print(
            f"Raw surge range: {np.min(reshaped_surge):.3f}m to {np.max(reshaped_surge):.3f}m"
        )
    ...
    def _get_raw_max_surge_data(self):
        ...
        try:
            ...
            print(
                f"Loading {len(member_files)} ensemble members for raw max calculation..."
            )
    ...
    def _create_interpolated_surge_visualization(
        self, output_path: Path, interpolated_surge: np.ndarray
    ):
        """Create visualization of interpolated surge heights."""
        try:
            import matplotlib.pyplot as plt
            ...
            print(
                f"DEBUG VIS: Interpolated surge range: {np.min(interpolated_surge):.3f} to {np.max(interpolated_surge):.3f}"
            )
            print(f"DEBUG VIS: Cells with surge > 0: {np.sum(interpolated_surge > 0)}")
            print(
                f"DEBUG VIS: Cells with surge > 0.1: {np.sum(interpolated_surge > 0.1)}"
            )
            print(
                f"DEBUG VIS: Cells with surge > 0.5: {np.sum(interpolated_surge > 0.5)}"
            )
            ...
            print(
                f"DEBUG VIS: Grid bounds: {minx:.3f} to {maxx:.3f} W, {miny:.3f} to {maxy:.3f} N"
            )
            ...
            print(
                f"DEBUG VIS: Expected interpolated shape: {len(lats)-1} x {len(lons)-1}"
            )
            ...
            print(
                f"DEBUG VIS: Meshgrid shapes - lon: {lon_mesh.shape}, lat: {lat_mesh.shape}"
            )
            ...
            if interpolated_surge.shape != (len(lats) - 1, len(lons) - 1):
                print(
                    f"ERROR: Shape mismatch! Interpolated: {interpolated_surge.shape}, Expected: {(len(lats)-1, len(lons)-1)}"
                )
                return
            ...
            for i in range(0, min(5, len(lats) - 1), 10):
                for j in range(0, min(5, len(lons) - 1), 10):
                    if interpolated_surge[i, j] > 0:
                        print(
                            f"  Grid[{i},{j}] ({lats[i]:.3f}, {lons[j]:.3f}): {interpolated_surge[i, j]:.3f}m"
                        )
            ...
            for lat_val, lon_val in [
                ...
                print(
                    f"  ({lat_val:.3f}, {lon_val:.3f}) -> Grid[{lat_idx},{lon_idx}] ({lats[lat_idx]:.3f}, {lons[lon_idx]:.3f}): {grid_val:.3f}m"
                )
            ...
            ax1.set_title(
                "Interpolated Surge Heights - Linear Scale",
                fontsize=14,
                fontweight="bold",
            )
            ...
            im2 = ax2.pcolormesh(
                lon_mesh, lat_mesh, log_surge, cmap="Blues", shading="auto", alpha=0.8
            )
            ...
            ax2.set_title(
                "Interpolated Surge Heights - Log Scale", fontsize=14, fontweight="bold"
            )
    ...
    def _create_mean_max_visualizations(self, output_dir: Path):
        ...
        try:
            ...
            if mean_surge is None:
                print(
                    "Warning: Could not get raw surge data for mean/max visualizations"
                )
                return
            ...
            im1 = ax1.pcolormesh(
                lon_mesh, lat_mesh, mean_surge, cmap="Blues", shading="auto", alpha=0.8
            )
            plt.colorbar(im1, ax=ax1, label="Mean Surge Height (m)")
            nicaragua_gdf.plot(ax=ax1, edgecolor="black", facecolor="none", linewidth=2)
            ax1.set_title(
                "Mean Surge Heights (Ensemble Average)", fontsize=14, fontweight="bold"
            )
            ax1.set_xlabel("Longitude", fontsize=12)
            ax1.set_ylabel("Latitude", fontsize=12)
            ax1.grid(True, alpha=0.3)
            ...
            im2 = ax2.pcolormesh(
                lon_mesh, lat_mesh, max_surge, cmap="Reds", shading="auto", alpha=0.8
            )
            plt.colorbar(im2, ax=ax2, label="Max Surge Height (m)")
            nicaragua_gdf.plot(ax=ax2, edgecolor="black", facecolor="none", linewidth=2)
            ax2.set_title(
                "Max Surge Heights (Ensemble Maximum)", fontsize=14, fontweight="bold"
            )
            ax2.set_xlabel("Longitude", fontsize=12)
            ax2.set_ylabel("Latitude", fontsize=12)
            ax2.grid(True, alpha=0.3)
    ...
    def _get_max_surge_data(self):
        ...
        try:
            ...
            print(
                f"Loading {len(member_files)} ensemble members for max calculation..."
            )
    ...
    def _apply_inland_penetration_to_data(self, surge_data):
        ...
        # Simple surge extension approach
        max_extension_distance_km = get_config_value(
            self.config, "surge.max_inland_distance_km", 10.0
        )
        extended_surge = np.zeros(len(grid_lons))
