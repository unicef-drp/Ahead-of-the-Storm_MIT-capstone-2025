...
def main():
    ...
    for i, forecast_time in enumerate(forecast_times, 1):
        ...
        if len(daily_data) == 0:
            print(f"⚠️  No data found for {forecast_time.strftime('%B %d, %Y %H:%M')} UTC")
            continue
        
        print(f"📈 Data points: {len(daily_data):,}")
        print(f"🎲 Ensemble members: {daily_data['ensemble_member'].nunique()}")
        print(f"⏰ Lead times: {daily_data['lead_time'].nunique()} (0-{daily_data['lead_time'].max()} hours)")
    ...
    print(f"{'='*70}")
    print("📁 All plots saved in: data/weatherlab/plots/")
    print(f"📊 Generated {len(forecast_times)} forecast dashboard sets")
    print("🎲 Each dashboard shows all ensemble members for that specific forecast time")
    print("⏰ Covers all 6-hour intervals (00:00, 06:00, 12:00, 18:00 UTC) from Nov 4-10, 2024")
    print(f"{'='*70}")
