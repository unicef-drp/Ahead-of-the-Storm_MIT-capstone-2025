...
class HurricaneDownloader:
    ...
    def _setup_directories(self):
        ...
        output_dir = download_config.get('output_directory', 'data/weatherlab/raw_ensemble')
        ...
        processed_dir = processing_config.get('processed_output_directory', 'data/weatherlab/processed')
    ...
    def _generate_forecast_times(self) -> List[datetime]:
        ...
        forecast_times = hurricane_config.get('forecast_times', ["00:00", "06:00", "12:00", "18:00"])
        ...
        self.logger.info(f"Generated {len(forecast_datetimes)} forecast times from {start_date} to {end_date}")
    ...
    def _save_failed_download(self, url: str, error: str):
        ...
        error_config = self.config.get('error_handling', {})
        failed_file = error_config.get('failed_downloads_file', 'logs/failed_downloads.txt')
    ...
    def download_hurricane_data(self) -> Dict[str, int]:
        ...
        self.logger.info(f"Starting download of hurricane data for {len(forecast_times)} forecast times")
        ...
        for forecast_dt in forecast_times:
            ...
            if self._download_file(url, output_path):
                ...
                failed_downloads += 1
                if self.config.get('error_handling', {}).get('save_failed_downloads', True):
    ...
    def _clean_hurricane_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'track_id' in df_cleaned.columns:
            ...
            self.logger.info(f"Filtered for {target_track_id}: {original_count} -> {filtered_count} records")
        ...
        available_sort_columns = [col for col in sort_columns if col in df_cleaned.columns]
