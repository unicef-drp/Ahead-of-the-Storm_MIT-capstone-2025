...
class HurricaneAnalyzer:
    ...
    def __init__(self, config_path: str = "config/hurricane_config.yaml"):
        ...
        processed_dir = self.config.get('processing', {}).get('processed_output_directory', 'data/weatherlab/processed')
    ...
    def _setup_logging(self):
        ...
        log_file = log_dir / f"hurricane_analyzer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def extract_forecast_dates(self, df: pd.DataFrame) -> List[datetime]:
        ...
        if 'forecast_time' in df.columns:
            ...
            forecast_dates = [datetime.combine(date, datetime.min.time()) for date in forecast_dates]
            forecast_dates.sort()
    ...
    def filter_by_forecast_date(self, df: pd.DataFrame, forecast_date: datetime) -> pd.DataFrame:
        """
        ...
        if 'forecast_time' in df.columns:
            ...
            self.logger.info(f"Filtered to forecast date {forecast_date.date()}: {len(filtered_df)} rows")
            return filtered_df
        else:
            self.logger.warning("No 'forecast_time' column found in data")
            return df
    ...
    def filter_by_ensemble_member(self, df: pd.DataFrame, member: str) -> pd.DataFrame:
        ...
        if 'ensemble_member' in df.columns:
            ...
            self.logger.info(f"Filtered to ensemble member {member}: {len(filtered_df)} rows")
            return filtered_df
        else:
            self.logger.warning("No 'ensemble_member' column found in data")
            return df
    ...
    def filter_by_lead_time(self, df: pd.DataFrame, lead_time: int) -> pd.DataFrame:
        ...
        if 'lead_time' in df.columns:
            ...
            self.logger.info(f"Filtered to lead time {lead_time}h: {len(filtered_df)} rows")
            return filtered_df
        else:
            self.logger.warning("No 'lead_time' column found in data")
            return df
    ...
    def prepare_track_data(self, df: pd.DataFrame, forecast_date: Optional[datetime] = None) -> pd.DataFrame:
        """
        ...
        if missing_columns:
            raise ValueError(f"Missing required columns for track plotting: {missing_columns}")
        ...
        if track_df['lead_time'].dtype == 'object':
            track_df['lead_time'] = pd.to_numeric(track_df['lead_time'], errors='coerce')
        
        self.logger.info(f"Prepared track data: {len(track_df)} points, {track_df['track_id'].nunique()} tracks")
        return track_df
    
    def prepare_intensity_data(self, df: pd.DataFrame, forecast_date: Optional[datetime] = None) -> pd.DataFrame:
        """
        ...
        if missing_columns:
            raise ValueError(f"Missing required columns for intensity analysis: {missing_columns}")
        
        # Sort by ensemble member and lead time
        intensity_df = df.sort_values(['ensemble_member', 'lead_time'])
        
        # Convert numeric columns
        intensity_df['wind_speed'] = pd.to_numeric(intensity_df['wind_speed'], errors='coerce')
        intensity_df['pressure'] = pd.to_numeric(intensity_df['pressure'], errors='coerce')
        intensity_df['lead_time'] = pd.to_numeric(intensity_df['lead_time'], errors='coerce')
        
        # Remove invalid values
        intensity_df = intensity_df.dropna(subset=['wind_speed', 'pressure', 'lead_time'])
        
        self.logger.info(f"Prepared intensity data: {len(intensity_df)} points")
        return intensity_df
    
    def calculate_ensemble_spread(self, df: pd.DataFrame, forecast_date: Optional[datetime] = None) -> pd.DataFrame:
        """
        ...
        spread_stats = df.groupby(group_columns).agg({
        ...
        spread_stats.columns = ['_'.join(col).strip('_') for col in spread_stats.columns.values]
        
        self.logger.info(f"Calculated ensemble spread for {len(spread_stats)} lead times")
        return spread_stats
    
    def save_analysis_results(self, df: pd.DataFrame, output_path: str, analysis_type: str = "general"):
        """
...
def main():
    ...
    try:
        ...
        output_dir = "../data/weatherlab/analysis"
        analyzer.save_analysis_results(track_data, f"{output_dir}/track_data.csv", "track")
        analyzer.save_analysis_results(intensity_data, f"{output_dir}/intensity_data.csv", "intensity")
        analyzer.save_analysis_results(ensemble_spread, f"{output_dir}/ensemble_spread.csv", "ensemble_spread")
