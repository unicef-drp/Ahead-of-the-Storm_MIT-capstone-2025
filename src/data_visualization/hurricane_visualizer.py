...
class HurricaneVisualizer:
    ...
    def plot_ensemble_tracks(
        ...
        for i, member in enumerate(ensemble_members):
            ...
            if len(member_data) > 1:
                # Plot track line
                ax.plot(
                    member_data["longitude"],
                    member_data["latitude"],
    ...
    def plot_intensity_curves(
        ...
        if len(ensemble_members) > 1:
            stats_data = (
                df.groupby("valid_time")
                .agg({"wind_speed": ["mean", "std"], "pressure": ["mean", "std"]})
                .reset_index()
            )
            ...
            if "wind_mean" in stats_data.columns:
                ax1.plot(
                    stats_data["valid_time"],
                    stats_data["wind_mean"],
                    color="black",
                    linewidth=3,
                    label="Ensemble Mean",
                )
    ...
    def plot_ensemble_spread(
        ...
        ax4.fill_between(
            stats_data["valid_time"],
            stats_data["pressure_min"],
            stats_data["pressure_max"],
            alpha=0.1,
            color="purple",
            label="Min-Max",
        )
    ...
    def _plot_spread_subplots(self, df: pd.DataFrame, axes):
        ...
        for i, (ax, var, color, title, ylabel) in enumerate(
            zip(axes, variables, colors, titles, ylabels)
        ):
    ...
    def plot_single_trajectory(
        ...
        save_plot: bool = True,
        show_plot: bool = False,
        show_nicaragua_boundary: bool = True,
    ) -> str:
        """
        ...
        Args:
            ...
            save_plot: Whether to save the plot
            show_plot: Whether to display the plot
            show_nicaragua_boundary: Whether to show Nicaragua boundary
        ...
        if len(filtered_df) == 0:
            self.logger.warning(
                f"No data found for forecast {forecast_time} and ensemble member {ensemble_member}"
            )
            return ""

        # Ensure valid_time is datetime
        if filtered_df["valid_time"].dtype == "object":
            filtered_df["valid_time"] = pd.to_datetime(
                filtered_df["valid_time"], format="mixed"
            )
            ...
            try:
                ...
                if nic_poly is not None:
                    nicaragua_gdf = gpd.GeoDataFrame(
                        geometry=[nic_poly], crs="EPSG:4326"
                    )
                    nicaragua_gdf.plot(
                        ax=ax1, color="none", edgecolor="black", linewidth=2, alpha=0.8
                    )
                    self.logger.info("Added Nicaragua boundary to plot")
            except Exception as e:
                self.logger.warning(f"Could not load Nicaragua boundary: {e}")
        ...
        ax1.set_title(
            f'Single Trajectory - Forecast: {forecast_time.strftime("%Y-%m-%d %H:%M")} UTC, Ensemble: {ensemble_member}'
        )
        ax1.grid(True, alpha=0.3)
...
def main():
    ...
    try:
        ...
        for i, forecast_date in enumerate(
            ...
            print(
                f"Processing forecast date {i+1}/{min(3, len(forecast_dates))}: {forecast_date}"
            )
