...
def main():
    ...
    try:
        ...
        print(
            f"   Lead times: {len(df['lead_time'].unique())} (0-{df['lead_time'].max():.0f} hours)"
        )
        ...
        if len(rafael_data) == 0:
            ...
            print(
                "   This might indicate the hurricane didn't form during the forecast period"
            )
            return
        ...
        print(
            f"   Forecast dates: {len(rafael_data['forecast_time'].dt.date.unique())}"
        )
        ...
        tracks_plot = visualizer.plot_ensemble_tracks(
            rafael_data, save_plot=True, show_plot=False
        )
        ...
        intensity_plot = visualizer.plot_intensity_curves(
            rafael_data, variable="wind_speed", save_plot=True, show_plot=False
        )
        ...
        print(f"\n📊 Track Statistics:")
        print(
            f"   Average track length: {track_stats.get('avg_track_length', 0):.1f} hours"
        )
        ...
        print(
            f"   Average ensemble spread: {track_stats.get('avg_ensemble_spread', 0):.1f} km"
        )
